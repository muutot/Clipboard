...
`Alt+V`）一键唤起，支持 **全文搜索（含中文分词）**、 **图片 OCR 文字识别**、收藏、编辑、快速粘贴等操作。
...
- **离线 OCR**：PP-OCR 引擎本地运行，无需联网（模型下载除外，见上方隐私承诺）
- **剪贴板暂停**：一键暂停/恢复记录
...
- **本地 API**：HTTP 接口供外部脚本调用；需携带 `Authorization: Bearer <token>`（token 位于 `conf/api.token`），仅接受
  `127.0.0.1`/`localhost` 的 Host 头并拒绝带 Origin 的浏览器请求
...
> **注意**：非 Windows 平台使用 500ms 轮询检测剪贴板变更：先比对文本，无文本时再比对文件路径与图片内容，因此图片/文件复制也能被采集；且不检查自触发标记，可能产生写入回环。
