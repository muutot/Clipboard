...
> 1.6 功能阈值：除还债外，新增**托盘速粘**、**自动标签规则**与**悬浮剪贴板**（置顶窄窗 + 默认位置五档 + 全局分组快捷键 + 标题栏拖拽）三个用户可见功能（标签页新增用户可配规则区，非系统内置）。
...
- [x] 自动标签规则（标签页设置项）：标签设置区新增规则编辑器（增删行 + 显式保存 + 后端错误回显，中英双语 `tags.autoTag*`，`settings-search` 关键词扩展），非内置写死。验收：`check` 0 错误 + 双语键齐 + 设置契约（类型/默认/校验/持久化）走完。
- [x] 悬浮剪贴板：独立置顶窄窗口（`routes/float`，仅全部/收藏分组，点击复制复用 `copyClipboardItem`），主窗口工具栏 + 托盘双入口。验收：`check`/`build` 通过 + 双语键齐（主窗口复制流需手工回归：`copyItem` 已收敛为共享服务的薄包装）。
