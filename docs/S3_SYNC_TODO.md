...
9. 首次开启同步必须处理存量数据。本地已有记录不会自动进入 outbox；首台设备发布全量基线，后续设备先引导再只上传独有增量，并与远端快照做首轮合并。
...
1. 开启同步时，本地存量记录不会自动出现在 outbox；第一台设备通过条件写认领 scope 初始化，并发布全量初始快照（或批量 segment）作为基线。
2. 后续设备先下载最新基线快照做首轮合并，只上传远端没有或版本更旧的独有增量，不重复上传全量。
3. 同一 id 按版本仲裁，同内容不同 id 进别名表，只保留胜者；合并后按 scope 容量/保留规则驱逐并登记 tombstone，再进入常规增量流程。
4. 收敛后生成合并快照作为新基线，旧初始 segment 在快照确认 + peer ack 后 GC。
5. 本地库从备份恢复或 schema 重置后，同步游标可能回退；检测到游标缺失或落后时回到快照引导，不信任本地状态。
...
| 驱逐流量       | 普通驱逐每条 1 个 delete op；大范围驱逐只发布 1 个驱逐标记，由快照固化；重复标记按版本去重 |
| 首次启用       | 首台上传 1 个初始快照；后续设备先引导只上传独有增量；收敛后生成 1 个合并快照               |
...
- [ ] 定义首次开启同步流程：首台设备认领初始化并发布全量基线；后续设备先引导再只上传独有增量；收敛后生成合并快照；明确冲突与容量驱逐顺序。
  - 验收：3 台已有 100k 记录的设备先后开启同步，最终一致且无丢失；后续设备不重复上传全量。
...
- [ ] scope 初始化与更新：`meta/scope.json` 用条件写创建，记录初始化状态（如首台设备/`initialized`）；配置变更以 version 单调递增覆盖，多台设备同时首次开启不互相覆盖。
  - 验收：并发初始化测试只有一个 scope 配置生效；首台认领成功，后续设备进入引导 + 补差流程；配置更新不会回退 version。
...
- [ ] 首次开启存量种子：首台设备发布全量初始快照；后续设备先下载基线、只上传独有增量；收敛后生成合并快照并 GC 初始 segment。
  - 验收：第一台 100k 存量只上传 1 个初始快照 + 缺失 blob；后续设备只上传独有增量；合并无丢失。
