...
The provider-neutral engine, frozen wire DTOs and codecs live in the Tauri-independent
`clipboard-sync` crate. The desktop application implements `SyncRepository` over SQLite and
converts its storage/domain record into the wire DTO explicitly; future local schema
