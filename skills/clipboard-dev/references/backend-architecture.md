...
- `v1/` contains the isolated, deliberately incompatible S3-first protocol primitives described in `docs/SYNC_V1.md`; it is not yet connected to `commands/sync/mod.rs`. `layout.rs` constructs and parses only canonical UUID/fixed-width/content-addressed keys below `v1/`, and its obsolete-object selector accepts only top-level `baseline-*`/`oplog-*` objects plus safe descendants of the discarded `resources/` prefix while always excluding `v1/`. `remote.rs` defines a provider-neutral object-store contract and an S3 adapter scoped to one validated configured prefix; callers see relative full keys, cursor listing cannot escape its requested prefix, writes support create-only/CAS conditions, and obsolete cleanup lists the complete scope then precisely deletes only selector-approved keys. Cleanup remains retryable after a partial transport failure and local state may be marked prepared only after its report returns successfully. `resources.rs` fingerprints regular files below a managed root with a bounded streaming read, skips content-addressed objects already confirmed by HEAD, publishes new objects with create-only file PUT, and materializes downloads through a bounded hash-verifying temporary file before replacing the local cache. Valid cache files are reused; corrupted or oversized regular cache files are repaired, while symlink/non-file destinations are rejected. `wire.rs` defines head/snapshot/segment/checkpoint records and wraps each bincode payload in a versioned magic envelope after zstd compression. Optional AES-256-GCM uses one PBKDF2-derived, remote-scope-specific `SessionKey` per run and a fresh nonce per object; encrypted payloads authenticate their header and wrong passwords/corruption are hard failures with no plaintext fallback. Decode paths enforce stored/uncompressed size bounds, object kind/version, exact bincode consumption, and unknown-flag rejection.
