...
`src-tauri/src/sync/` owns S3-first cross-device synchronization. The registered runtime uses
only `s3.rs`, `v1/`, and `commands/sync/`; the historical backup/WebDAV/pool modules are
unreachable and scheduled for physical deletion in the next cleanup unit.

- `v1/` is the first and only supported replication protocol described in `docs/SYNC_V1.md`. `commands/sync/mod.rs` snapshots S3 settings without holding the config lock during I/O, derives one optional remote-scoped `SessionKey`, constructs one prefix-scoped `S3ObjectStore`, and calls `v1::sync_database` for both `sync_now` and automatic runs. The engine prepares the configured remote by precisely deleting old top-level baseline/oplog/resource objects, publishes an upload-first bootstrap snapshot, bounded immutable segments and the local device head, then pulls other heads and advances each cursor only with its successfully applied SQLite transaction. Resource files are streamed, content addressed and hash verified; wrong passwords/corruption are hard failures; an idle run uploads no objects.
...
- `commands/sync/mod.rs` exposes only `get_sync_config`, `set_sync_config`, typed S3 connection testing, and `sync_now`. The current config provider values are `off`/`s3`; secrets are write-only; segment and resource limits are current v1 policy. `SyncRunResult` reports entry/resource/object and byte counters from the v1 engine. A global try-lock serializes manual/automatic runs, successful remote application emits `clipboard-history-invalidated`, and status persistence is best-effort after either outcome.
- `commands/sync/auto.rs` is the auto-sync background worker (`AutoSyncWorker`), started in `lib.rs` setup and managed as `Mutex<Option<AutoSyncWorker>>`. It re-reads `auto_sync()` and `auto_sync_interval_secs()` every second and calls the same `run_sync` entry point as `sync_now`; shutdown retains the stop flag and joins the worker thread.
