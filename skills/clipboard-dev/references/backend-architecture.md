...
- `v3/` contains the isolated, deliberately incompatible S3-first protocol primitives described in `docs/SYNC_V3.md`; it is not yet connected to `commands/sync/mod.rs`. `layout.rs` constructs and parses only canonical UUID/fixed-width/content-addressed keys below `v3/`, and its legacy-cleanup selector accepts only top-level `baseline-*`/`oplog-*` objects plus safe descendants of the old `resources/` prefix while always excluding `v3/`. `wire.rs` defines head/snapshot/segment/checkpoint records and wraps each bincode payload in a versioned magic envelope after zstd compression. Optional AES-256-GCM uses one PBKDF2-derived, remote-scope-specific `SessionKey` per run and a fresh nonce per object; encrypted payloads authenticate their header and wrong passwords/corruption are hard failures with no plaintext fallback. Decode paths enforce stored/uncompressed size bounds, object kind/version, exact bincode consumption, and unknown-flag rejection.
