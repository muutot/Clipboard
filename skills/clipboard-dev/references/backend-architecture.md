...
`Database` wraps a rusqlite connection configured for foreign keys, WAL, normal synchronous mode, memory temp storage, cache, and mmap. Repository traits/implementations cover clipboard, OCR, search-outbox, and sync-state behavior. `Database::from_connection` creates the schema and then ensures `sync_metadata.device_id` is a persisted UUID before returning the database; any non-UUID value is replaced directly and is not retained as a compatibility alias.

`storage/sync_state.rs` is the new compact local replication contract, currently dormant until the v1 command orchestrator calls `Database::initialize_sync`. Initialization preserves clipboard rows as first-snapshot input, directly discards obsolete sync-only state without reading or converting it, assigns deterministic `(modified_at_ms, sync_writer_device_id)` versions, and enables guarded v1 triggers. Those triggers append only small id/version outbox rows and maintain a separate winning tombstone per deleted id; snapshot/outbox export batch-loads current rows and coalesces repeated ids instead of storing a second full copy of the 100,000-row dataset. Local snapshot/segment publication state acknowledges its outbox prefix atomically, while remote snapshot/segment apply updates records, aliases, tag mirrors, tombstones and the source-device cursor in one transaction. Equal/older versions and exact segment replays are no-ops; an epoch/sequence/key regression or any SQL error rolls the complete batch back. The obsolete changelog and old modified-time triggers stop writing once v1 is initialized.
...
- `v1/` contains the isolated, deliberately incompatible S3-first protocol described in `docs/SYNC_V1.md`; it is not yet connected to `commands/sync/mod.rs`. `layout.rs` constructs and parses only canonical UUID/fixed-width/content-addressed keys below `v1/`, and its obsolete-object selector accepts only top-level `baseline-*`/`oplog-*` objects plus safe descendants of the discarded `resources/` prefix while always excluding `v1/`. `remote.rs` defines a provider-neutral object-store contract and an S3 adapter scoped to one validated configured prefix; callers see relative full keys, cursor listing cannot escape its requested prefix, writes support create-only/CAS conditions, and obsolete cleanup lists the complete scope then precisely deletes only selector-approved keys. Cleanup remains retryable after a partial transport failure and local state may be marked prepared only after its report returns successfully. `resources.rs` rewrites machine-local item/metadata paths to canonical resource keys, fingerprints regular files below managed roots with bounded streaming reads, uploads each new content object once, and hash-verifies downloads through temporary files before rewriting received records to this device's managed roots. `engine.rs` publishes an upload-first bootstrap snapshot, bounded immutable segments and the local device head, then pulls every other head and advances its per-device SQLite cursor only with the successfully applied pack; two independently populated databases and later incremental changes converge in provider-neutral tests, while an idle run uploads nothing. `wire.rs` defines head/snapshot/segment/checkpoint records and wraps each bincode payload in a v1 magic envelope after zstd compression. Optional AES-256-GCM uses one PBKDF2-derived, remote-scope-specific `SessionKey`; immutable nonces are derived from the authenticated header plus compressed plaintext so retries reproduce the exact content-addressed ciphertext, while password mismatch/corruption is a hard failure with no plaintext fallback. Decode paths enforce stored/uncompressed size bounds, object kind/version, exact bincode consumption, and unknown-flag rejection.
