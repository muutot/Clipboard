...
- `backup.rs` builds a baseline ZIP (`baseline-{device_id}-{timestamp}.zip`) from the clipboard database via `create_baseline_backup`, reads it back with `read_baseline_items`/`read_baseline_with_resources`, and produces a `BackupManifest` through `read_manifest_from_backup`. `create_oplog_backup`/`mark_oplog_synced`/`purge_oplog`/`count_unsynced` manage the outbox so only unsynced changes are uploaded next run. Since item ids are content-derived (`img_{hash}`, `file_{hash}`, `files_{group_hash}`, `{text_hash}_{ms}`), `import_baseline_items` uses last-write-wins (`WHERE excluded.modified_at_ms >= clipboard_items.modified_at_ms`) so a stale baseline never overwrites a newer local edit; subsequent conflicts converge through `apply_remote_oplog`'s LMW.
- `wire.rs` wraps the wire formats: `Baseline` (format_version, timestamps, device/app info, items, plus inline `OplogResource` bytes since format v2) and `Oplog` (entries plus inline `OplogResource` bytes) are bincode v2-encoded envelopes via `serialize_baseline_with_resources`/`deserialize_baseline_with_resources` (a `BaselineV1` struct preserves the legacy items-only layout; `deserialize_baseline_with_resources` tries v2 first and falls back to v1) and `serialize_oplog_with_resources`/`deserialize_oplog_with_resources`. `merge_baselines` unions several disjoint baseline payloads (items by id keeping the newer `created_at_ms`, resources by wire path) — multiple baselines arise only from concurrent first syncs and have no common root, so they are merged into a superset rather than dropped. `write_baseline_zip` writes a baseline ZIP from already-serialized wire-form items + resources (used by the consolidation path). Despite the old module name, the on-wire format is bincode, not protobuf; there is no `.proto` file. Zip entries are named `baseline.bin`/`oplog.bin`. The V2 bincode envelope falls back to the entries-only V1 layout when reading files written before inline resources shipped; there is no JSON serialization of oplog payloads. Rollover merges parse existing remote oplogs with `deserialize_oplog_with_resources` (V1 fallback returns entries with no resources).
- `resources.rs` rewrites entry and item paths between local and portable wire form (`category/relative`: `image/`, `file/`, `preview/`, `icon/`) and carries the referenced file bytes inline as `OplogResource`. `collect_entry_resources` (oplog upload side) and `collect_item_resources` (baseline side) read bytes and rewrite to wire paths; `materialize_resources` + `rewrite_to_local`/`rewrite_item_paths_to_local` (download side) write bytes into the receiver's storage subdirectories and restore absolute paths before applying. Files that cannot be read are cleared from the entry; resources dedupe by wire path (an image whose preview equals its resource is transferred once).
