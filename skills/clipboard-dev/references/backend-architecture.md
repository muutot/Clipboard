...
- `commands/sync/mod.rs` orchestrates: first sync downloads or uploads the baseline, then merges/rolls over remote oplogs (collecting resource bytes for new local entries and re-serializing with the combined resource set), materializes downloaded resources and applies remote entries via `database.apply_remote_oplog`, and prunes old remote oplog files. Sync commands snapshot all needed config once into a `SyncSettings` struct under the `ConfigStore` lock and release the lock before any network I/O (re-acquiring only to record `update_sync_status`), so a slow remote never blocks other config access. `sync_download_backup` fetches a remote backup file for local restore; `verify_backup_file` validates its manifest.
