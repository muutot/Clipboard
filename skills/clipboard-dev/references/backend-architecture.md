...
- `backup.rs` builds a baseline ZIP (`baseline-{device_id}-{timestamp}.zip`) from the clipboard database via `create_baseline_backup`, reads it back with `read_baseline_items`, and produces a `BackupManifest` through `read_manifest_from_backup`. `create_oplog_backup`/`mark_oplog_synced`/`purge_oplog`/`count_unsynced` manage the outbox so only unsynced changes are uploaded next run. Because item ids are content-derived, `import_baseline_items` uses last-write-wins (`WHERE excluded.modified_at_ms >= clipboard_items.modified_at_ms`) so a stale baseline never overwrites a newer local edit; subsequent conflicts converge through `apply_remote_oplog`'s LMW.
