...
- `proto.rs` wraps the wire formats: `Baseline` (format_version, timestamps, device/app info, items) and `Oplog` (entries) are bincode v2-encoded envelopes via `serialize_baseline`/`deserialize_baseline`/`serialize_oplog`/`deserialize_oplog`; `protos/clipboard.proto` documents the intended schema. Zip entries are named `baseline.pb`/`oplog.pb`. Remote oplog readers fall back to JSON so files written by earlier builds stay readable; rollover merges parse existing remote oplogs as JSON (`serde_json::from_str::<Vec<SyncChangeLogEntry>>`).
