...
- `commands/sync/mod.rs` orchestrates each provider under a stable SHA-256 remote scope derived from provider, endpoint, remote path, and non-secret account/bucket fields. Baseline initialization and remote statistics live in `sync_remote_state`, so switching providers or paths cannot inherit another remote's progress; `lastSyncMs` is status/display data only. On the first sync for a scope it uploads a fresh baseline when the remote has none, or downloads **every** remote baseline, merges the unpacked contents via `merge_baseline_archives`, absorbs/materializes their scoped pool references, optionally consolidates redundant baselines, and imports the merged items. Local changes are uploaded as immutable objects named `oplog-{device_id}-s{first_sequence}-e{last_sequence}-{sha256}` instead of appending to an existing remote object. Successfully processed immutable objects are recorded per scope/name/revision in `sync_applied_oplogs`; legacy timestamp-named objects are deliberately downloaded every run because an older client may overwrite them without a trustworthy metadata revision. The current retention cleanup still prunes old remote oplogs by count, and manual `sync_compact_remote` still refreshes a baseline then removes older baseline/oplog files; these deletion paths require a separate acknowledgement-safety pass before they are safe for long-offline devices. Sync commands snapshot all needed config once into a `SyncSettings` struct under the `ConfigStore` lock and release the lock before any network I/O (re-acquiring only to record `update_sync_status`), while `SYNC_RUN_LOCK: Mutex<()>` serializes manual and automatic runs. `sync_download_backup` fetches a remote backup file for local restore; `verify_backup_file` validates its manifest.
