...
| `SettingEntry.svelte` | Generic settings row/card renderer. Takes a `SettingEntryConfig` union prop (`number`/`size`/`slider`/`toggle`/`select`/`text`/`heading`, defined in `src/lib/types/settings-entry.ts`) and renders the approved shared card/row markup with the shared `--settings-*` primitives. Configs carry `get`/`set` (and `getUnit`/`setUnit` for `size`) plus optional `oninput`/`onchange`/`onblur`, so panels describe a row declaratively instead of duplicating markup. Panels keep `desc`-style copy and `aria-label`s in the config; localize at the call site. `size` entries reuse `SIZE_UNIT_OPTIONS` from `src/lib/utils/size.ts`. The component owns the row-layout and unit-select scoped styles it needs; shared card/toggle/slider/select visuals stay in `settings-shared.css`. |
