...
- `.setting-card-row` action-button rules target direct child buttons only, so nested `CustomSelect` triggers and popover options retain the same shared dropdown styling as the theme selector. Per-use select classes may change control dimensions (for example the `64px` unit selector), but must not redefine the trigger or list visual treatment.
