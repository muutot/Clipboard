...
- `sync_v3_remote_state`, `sync_v3_cursors`, and `sync_v3_remote_resources`: provider-scope-local publication state, per-device sequence cursors, and confirmed content-addressed resource objects. Their orchestration methods are introduced with the v3 command path; the tables already exist so activation and later cursor commits remain transactional.
