...
The only wire namespace is `v1/`, and every object has the `CLPSYNC1` magic plus format version `1`. `sync/v1/wire.rs` is authoritative for `DeviceHead`, `Snapshot`, `Segment`, checkpoint records, mutation batches, the compressed envelope, size limits, and exact bincode consumption.
