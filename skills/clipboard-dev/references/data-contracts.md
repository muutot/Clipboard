...
`set_sync_config` accepts `provider`, `endpoint`, `remotePath`, `username`, `password`, `autoSync`, `autoSyncIntervalSecs`, `maxRemoteOplogFiles`, `oplogRolloverEntries`, `oplogRolloverSizeBytes`, `maxSyncImageBytes`, `maxSyncFileBytes`, `s3Region`, `s3Bucket`, `s3AccessKey`, `s3SecretKey` (all optional where applicable); `password`/`s3SecretKey` fall back to the previously stored value when omitted so the UI never needs to send secrets back. `get_sync_config` additionally returns `compactionSuggested` (bool), derived from `sync_metadata` keys `sync_remote_oplog_count` / `sync_remote_baseline_modified_ms` recorded during the last sync: the UI offers a manual compaction when the remote oplog count is at/above `maxRemoteOplogFiles` or the newest baseline is older than 30 days. `sync_compact_remote` (command: runs a full `run_sync` first, then writes/uploads a fresh baseline-with-resources and deletes all older baselines plus oplogs trimmed to the retention ceiling) is the only compaction entry point — manual only, never scheduled. `test_sync_connection` takes the same provider/endpoint/remote-path/credential inputs and returns a serialized `WebDavTestResult`/`S3TestResult` JSON string. Remote oplog files are named `oplog-{device_id}-{timestamp}` (no extension) and hold `Oplog` envelopes serialized via `sync::wire::serialize_oplog_with_resources` (bincode v2). Baseline zips embed `baseline.bin` plus a human-readable `manifest.json`; since `Baseline` format v2 a baseline also carries inline `OplogResource` bytes so a downloaded baseline is self-sufficient on a new device (legacy items-only `BaselineV1` still decodes via `deserialize_baseline_with_resources`). Oplogs and baselines carry inline `OplogResource` bytes (`category/relative` wire paths: `image/`, `file/`, `preview/`, `icon/`) so receivers materialize resources into their own storage layout before `apply_remote_oplog`/`import_baseline_items`. `sync::resources::{collect_entry_resources, collect_item_resources, materialize_resources, rewrite_to_local, rewrite_item_paths_to_local}` rewrite entry/item paths between absolute/local and wire form; `merge_baselines` unions multiple downloaded baseline payloads (items by id keeping the newer `created_at_ms`, resources by wire path) and `write_baseline_zip` persists the merged superset so concurrent disjoint baselines converge into one. `deserialize_oplog_with_resources`/`serialize_oplog_with_resources` handle inline bytes; the V2 bincode envelope falls back to the entries-only V1 layout when reading files written before inline resources shipped. Item ids are content-derived (`img_{hash}`, `file_{hash}`, `files_{group_hash}`, `{text_hash}_{ms}`), so the same content copied on two devices produces the same id. `apply_remote_oplog` resolves conflicts last-write-wins on `modified_at_ms` (update/delete only apply when the remote entry is at least as new as the local row; insert is `INSERT OR IGNORE`). `import_baseline_items` also guards its `ON CONFLICT(id) DO UPDATE` with `WHERE excluded.modified_at_ms >= clipboard_items.modified_at_ms`, so a stale baseline snapshot never clobbers a newer local edit (the `clipboard_items_set_modified` trigger bumps `modified_at_ms` on any update). See `commands/sync/mod.rs` and the `sync/` module in `backend-architecture.md` for transport behavior.
