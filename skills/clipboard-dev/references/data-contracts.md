...
PPaste backups made on Windows store zip entry names with backslashes (e.g. `PasteData\1745.png`); `read_zip_entry` tries both `/` and `\` separators so images are found regardless of the archive's separator convention.
