...
- `clipboard_items`: ID, kind, title/text/html/rtf/resource/preview, `content_hash`, source/icon, size/time, favorite, soft-delete fields, and metadata JSON. `(kind, content_hash)` is unique. Sync v1 adds `modified_at_ms` plus `sync_writer_device_id`; together they form the deterministic `(timestamp, writer UUID)` conflict version. The HTML/RTF/version columns are added idempotently via `ensure_column` for databases created before their introduction.
...
- `sync_metadata`: key/value store. `device_id` is a random v4 UUID created by `Database::from_connection` and persisted for the lifetime of the local database; any missing or non-UUID value is replaced directly and no historical identity alias is retained. The store also carries the temporary `sync_suppress_changelog` flag used to silence sync triggers. The discarded `sync_changelog` triggers and `clipboard_items_set_modified` are disabled whenever `sync_enabled=1`, so v1 initialization cannot double-write both implementations or let the old wall-clock trigger replace a v1 version. The search triggers do not read either sync guard, so received items still enter `search_outbox`.
...
- `sync_tombstones`: one compact winning delete version per id. A soft delete creates/updates it, restore removes it, and permanent deletion retains it after the clipboard row is gone. Snapshot export returns all active versioned rows plus these tombstones in deterministic id order.
- `sync_publication_state`, `sync_cursors`, and `sync_remote_resources`: provider-scope-local publication state, per-device sequence cursors, and confirmed content-addressed resource objects. A local epoch is stable until `reset_sync_remote_state`; bootstrap/segment publication commits the snapshot/segment pointer and acknowledges only the covered outbox prefix in the same transaction. Bootstrap publication is rejected until the configured remote has been prepared by exact deletion of obsolete objects, and neither publication path may advance past the local AUTOINCREMENT high-water.

`Database::initialize_sync` initializes the first and only supported sync protocol. It does not read or convert baseline/oplog state: it preserves every `clipboard_items` row as first-snapshot input, assigns blank writer ids to the current database UUID, rebuilds recycle-bin tombstones, directly discards obsolete sync-only state, and sets `sync_protocol_version=1`/`sync_enabled=1`. Repeated calls are idempotent and do not clear pending v1 changes. The current command path does not invoke it until the S3-first orchestrator is connected.
...
`set_sync_config` accepts `provider`, `endpoint`, `remotePath`, `username`, `password`, `autoSync`, `autoSyncIntervalSecs`, `maxRemoteOplogFiles`, `oplogRolloverEntries`, `oplogRolloverSizeBytes`, `maxSyncImageBytes`, `maxSyncFileBytes`, `s3Region`, `s3Bucket`, `s3AccessKey`, `s3SecretKey`, and `syncPassword` (all optional where applicable); the three write-only secrets (`password`, `s3SecretKey`, `syncPassword`) fall back to their previously stored values when omitted/null so the UI never needs to send secrets back. `get_sync_config` returns the full effective sync policy plus `hasPassword` / `hasS3SecretKey` / `hasSyncPassword`, never the raw secrets. Its remote maintenance observations are read from the current SHA-256 remote scope in `sync_remote_state`, not from global timestamps. The compatibility field `compactionSuggested` now means “snapshot refresh suggested”: it is true only for an initialized remote whose newest baseline timestamp is missing or at least 30 days old; oplog count does not trigger it. `maxRemoteOplogFiles` is retained in config but not enforced until a per-device acknowledgement protocol can prove deletion safe. `sync_list_remote_backups` returns a directly serialized array of camelCase entries (`name`, `isDirectory`, `sizeBytes`, `modifiedMs`), not a JSON string containing that array. The compatibility-named `sync_compact_remote` command performs a full sync and appends a fresh baseline without deleting any existing baseline or oplog; the frontend exposes it as `syncRefreshRemoteSnapshot`. `test_sync_connection` takes the same provider/endpoint/remote-path/credential inputs and returns a serialized `WebDavTestResult`/`S3TestResult` JSON string. New remote oplogs are immutable objects named `oplog-{device_id}-s{first_sequence}-e{last_sequence}-{sha256}` (no extension) and hold `Oplog` envelopes serialized via `sync::wire::serialize_oplog_with_resources` (bincode v2); only the database-persisted UUID is consulted as the local device identity. Older timestamp-named objects remain readable and are reprocessed each run rather than trusted as immutable. Baseline ZIPs embed `baseline.bin` plus a human-readable `manifest.json`; downloaded archive bytes are unpacked by `read_baseline_archive_bytes`/`merge_baseline_archives` before the contained bincode payload reaches `deserialize_baseline_with_resources`. Pool manifests are stored per remote scope, so `sync::pool::prepare_pool_refs` only downgrades bytes confirmed on that same remote; `materialize_resources` fetches pool references through an optional `Pool` callback; `merge_baseline_contents` prefers inline bytes over pool references so merged payloads stay self-sufficient. `deserialize_baseline_with_resources`/`deserialize_oplog_with_resources` decode one canonical wire envelope each with no per-version fallback chain. Image and file ids are content-derived (`img_{hash}`, `file_{hash}`, `files_{group_hash}`); text/link ids are per-capture timestamp values that remain stable across later edits, so `sync_item_aliases` persistently coalesces different ids that first arrive with the same `(kind, content_hash)`. `apply_remote_oplog` and `import_baseline_items` resolve later updates/deletes through that alias and apply LWW against `COALESCE(modified_at_ms, created_at_ms)`. Remote application suppresses both changelog echo and the local modified-time trigger, preserving the source timestamp. Every recognized operation must execute successfully inside the same transaction; SQL/constraint failures or unknown operations roll back the entire batch, and the provider command returns an object-specific error without recording it in `sync_applied_oplogs`. See `commands/sync/mod.rs` and the `sync/` module in `backend-architecture.md` for transport behavior.

Sync resource paths are an untrusted contract even though `OplogResource` keeps its existing bincode fields. New uploads use `category/sha256-{raw_digest}.{safe_extension}` names. The shared parser permits only `image`, `preview`, `file`, and single-file `icon` paths and rejects absolute paths, traversal, backslashes, control/Windows-invalid names, and excessive depth before a local path, remote pool object, or manifest entry is formed. Upload reads must canonicalize into the configured managed root. Download materialization must reject symlink traversal, verify canonical hashes (plus compatible legacy image/file hashes), repair a corrupted verifiable cache only from validated pool bytes, and fail if the referenced regular managed file is unavailable. Resource validation/rewrite failures are object-specific sync failures and must not mark the oplog applied.
