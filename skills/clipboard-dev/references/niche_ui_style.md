...
| 已处理 | 内联布局             | Keyboard 配置卡的内联样式已改为命名 class（`.config-bar-actions`、`.config-bar-btn`，现位于 `KeyboardSettingsPanel.svelte`）                                                                                       | 新布局样式一律使用命名 class 并放入正确设置层级                               |
| 已处理 | Keyboard 层级        | Keyboard 配置卡已从 shell（breadcrumb 行与二级导航之间）移入 `KeyboardSettingsPanel.svelte`，作为“全局”分类的首张卡片；键盘分组页签顺序为 全局/条目操作/快速复制/切换                                              | 已遵守统一 shell 顺序；配置卡归面板内容区                                     |
| 已处理 | 设置页硬编码文案     | 设置搜索条目（Keyboard/OCR/统计/数据库维护等）的标题与描述已全部接入 i18n（`settings-search.ts` 引用 `keyboard.*`/`storage.*`/`statistics.*`/`settingsSearch.*` 键），`locales.test.ts` 全量扫描源码点分键防止回归 | 仅搜索别名关键词（中英混合搜索词）按设计保留字面量；新增条目一律走 i18n 键    |
