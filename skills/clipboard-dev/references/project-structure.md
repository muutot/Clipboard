...
| Float panel window        | `src/routes/float/+page.svelte`                                     | Standalone always-on-top narrow WebviewWindow (All/Favorites tabs, click-to-copy via shared `copyClipboardItem`); opened from the main toolbar or the tray menu; backend builds it hidden and the page reveals itself on mount (showing/focusing at build time wedged the first paint when the main window was in front) |
