...
| `src/lib/utils/`                     | Date query, keyboard target checks (`keyboard.ts`), global keydown decision table (`keyboard-actions.ts`), four-copy item sync funnel (`item-sync.ts`), bulk planning/snapshots (`bulk-actions.ts`), auto-clearing panel feedback (`feedback.svelte.ts`), theme application, time formatting, virtual-scroll calculations, modal focus traps (`focus.ts`), card height estimation (`card-height.ts`), search history persistence and suggestion slicing (`search-history.ts`), window bounds save/restore/migration controller (`window-bounds.ts`), history filtering (`history-filter.ts`), search-cache maintenance (`search-cache.ts`) |
