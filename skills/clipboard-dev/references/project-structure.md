...
| Float panel window        | `src/routes/float/+page.svelte`                                     | Standalone always-on-top narrow WebviewWindow (All/Favorites tabs, click-to-copy via shared `copyClipboardItem`); opened from the tray menu, its global hotkey, or the in-app shortcut; backend builds it hidden and the page reveals itself on mount (showing/focusing at build time wedged the first paint when the main window was in front) |
...
| `src/lib/services/`                  | Tauri invoke wrappers (guarded `invokeTauri`/`invokeTauriRequired` in `runtime.ts`), stores, persistence, record mapping, runtime detection                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `src/lib/types/`                     | Frontend contracts for clipboard, settings, runtime, and memory diagnostics                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `src/lib/utils/`                     | Date query, keyboard target checks (`keyboard.ts`), global keydown decision table (`keyboard-actions.ts`), four-copy item sync funnel + recycle-bin page merge (`item-sync.ts`), bulk planning/snapshots (`bulk-actions.ts`), auto-clearing panel feedback (`feedback.svelte.ts`), theme application, time formatting, virtual-scroll calculations, modal focus traps (`focus.ts`), card height estimation (`card-height.ts`), search history persistence and suggestion slicing (`search-history.ts`), window bounds save/restore/migration controller (`window-bounds.ts`), history filtering and backend filter args (`history-filter.ts`), search-cache maintenance (`search-cache.ts`) |
