...
| `src/lib/utils/`                     | Date query, keyboard target checks, theme application, time formatting, virtual-scroll calculations, modal focus traps (`focus.ts`), card height estimation (`card-height.ts`), search history persistence and suggestion slicing (`search-history.ts`) |
