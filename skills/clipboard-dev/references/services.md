...
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| `storage.ts`  | storage status/config, per-kind stats/deletion, path changes, search rebuild/validation, DB repair, performance, icon cache (list/delete/replace), export/import (`get_export_formats`, `get_import_formats` with `{id,label,extension}` entries, `export_to_file` with favorites/date-range/content-type filter options, `import_from_file`), and the S3-only sync boundary (`getSyncConfig`, object-based `setSyncConfig`, typed `testSyncConnection`, `runSync`) | nullable/empty values where declared |
...
Paste logic lives in `clipboard.ts` (`pasteClipboardItem`), not the route. When `pasteCleaningEnabled` is set, plain/format paste run `transform_text` with `cleanPaste` first; format paste falls back to writing the cleaned plain text (instead of HTML) whenever cleaning changed the content, since URL/whitespace cleanup cannot be applied reliably inside markup. `format` passes the stored `rtfContent` to `writeClipboardHtml(html, plainText, rtf)`, which writes `text/rtf` alongside `text/html`/`text/plain` so RTF-preferring apps keep formatted paste.

Double-click paste (`doubleClickPasteItem`, enabled by the `doubleClickPaste` setting which defaults on) resolves the loaded item and delegates to `pasteClipboardItem(item, "auto")`: text/link items prefer the HTML representation when the item has `htmlContent`, otherwise plain; image/file items first materialize (the same wrapper used by copy, detail, fullscreen and save), then image items re-fetch the verified local image via `convertFileSrc` and file items use the multi-file `textContent` JSON list or primary `resourcePath`. When `doubleClickPaste` is off, the card's double click falls back to `ondetail`.
