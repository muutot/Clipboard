...
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| `storage.ts`  | storage status/config, per-kind stats/deletion, path changes, search rebuild/validation, DB repair, performance, icon cache (list/delete/replace), export/import (`get_export_formats`, `get_import_formats` with `{id,label,extension}` entries, `export_to_file` with favorites/date-range/content-type filter options, `import_from_file`), and sync config/run/list/download wrappers (`sync_list_remote_backups` returns provider-neutral `RemoteBackupEntry[]` directly; `syncRefreshRemoteSnapshot` invokes the compatibility-named non-destructive snapshot command) | nullable/empty values where declared |
