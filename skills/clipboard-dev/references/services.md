...
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| `storage.ts`  | storage status/config, per-kind stats/deletion, path changes, search rebuild/validation, DB repair, performance, icon files, export/import (`get_export_formats`, `get_import_formats` with `{id,label,extension}` entries, `export_to_file` with favorites/date-range/content-type filter options, `import_from_file`) | nullable/empty values where declared |
