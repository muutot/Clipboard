...
| Capture          | Filter (ignored apps, max file copy / text capture size) → `IgnoredAppsSettingsPanel`; Sensitive content (local-only mode, sensitive patterns, recording pause) → `SensitiveContentSettingsPanel`; Icon cache → built into the parent         |
