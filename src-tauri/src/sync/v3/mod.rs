//! S3-first sync v3 primitives.
//!
//! This namespace is intentionally incompatible with the legacy baseline and
//! oplog protocol. Transport orchestration is added separately so the strict
//! object layout and wire envelope stay independently testable.

pub mod layout;
pub mod wire;

pub use layout::{
    checkpoint_object_key, head_object_key, legacy_object_candidate, parse_segment_key,
    resource_object_key, segment_object_key, segment_prefix, snapshot_object_key, ParsedSegmentKey,
    ResourceCategory, CHECKPOINT_HEAD_KEY, HEADS_PREFIX, V3_ROOT,
};
pub use wire::{
