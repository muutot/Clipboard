...
Function DataDirectoryPage
  ...
  !insertmacro MUI_HEADER_TEXT "Data Storage Location" "Choose where clipboard data will be stored"

  nsDialogs::Create 1018
  Pop $0

  ${NSD_CreateLabel} 0 0 100% 24u "Select the folder for clipboard data storage (database, images, search index):"
  Pop $0

  ${NSD_CreateDirRequest} 0 30u 75% 12u $DataDirectory
  Pop $R0
  ${NSD_OnChange} $R0 DataDirectoryPageOnChange

  ${NSD_CreateBrowseButton} 78% 28u 22% 15u "Browse..."
  Pop $R1
  ${NSD_OnClick} $R1 DataDirectoryPageBrowse

  ${NSD_CreateLabel} 0 50u 100% 30u "Clipboard data (history, images, search index) will be stored here.$\nYou can change this later in the app settings."
  Pop $0

  nsDialogs::Show
FunctionEnd

Function DataDirectoryPageOnChange
  Pop $R0
  ${NSD_GetText} $R0 $DataDirectory
FunctionEnd

Function DataDirectoryPageBrowse
  nsDialogs::SelectFolderDialog "Select Data Storage Folder" $DataDirectory
  Pop $0
  ${If} $0 != error
    StrCpy $DataDirectory $0
    ${NSD_SetText} $R0 $DataDirectory
  ${EndIf}
FunctionEnd
